...
# CONFIG
# =========================
YOUTUBE_LEFT = "https://www.youtube.com/watch?v=FYu2xy1MaZs"
YOUTUBE_RIGHT = "https://www.youtube.com/watch?v=8p-Bx1TbGV0"
...
WINDOW_NAME = "ROS2 YouTube Calib + 360"
...
YOUTUBE_STREAM_KEY = "c168-y5ph-2k06-hmb9-d299"
STREAM_FPS = 15
STREAM_BITRATE = "6M"
...
def get_direct_stream_url(youtube_url: str) -> str:
    ...
    raise RuntimeError(f"No se pudo resolver la URL directa: {last_error}")
...
class FFmpegStreamer:
    ...
    def start(self) -> None:
        ...
        if not self.stream_key or self.stream_key == "PON_AQUI_TU_NUEVA_CLAVE":
        ...
        cmd = [
            ...
            # audio silencioso, igual que en la prueba que sí funciona
            "-f", "lavfi",
...
class StreamReader:
    def __init__(self, youtube_url: str, name: str):
    ...
    def open(self) -> bool:
        try:
            self.direct_url = get_direct_stream_url(self.youtube_url)
            print(f"[{self.name}] URL directa resuelta.")
        except Exception as e:
            print(f"[{self.name}] Error resolviendo URL con yt-dlp: {e}")
            return False

        self.cap = cv2.VideoCapture(self.direct_url, cv2.CAP_FFMPEG)
        if not self.cap.isOpened():
            print(f"[{self.name}] No se pudo abrir el stream.")
            return False
        return True
...
class RawCaptureWorker:
    def __init__(self, reader: StreamReader, name: str, rotation_mode: Optional[str]):
        self.reader = reader
        self.name = name
...
class YouTubeCalibAndStitchNode(Node):
    def __init__(self) -> None:
        super().__init__("youtube_calib_and_stitch_node")
        ...
        self.reader_left = StreamReader(YOUTUBE_LEFT, "LEFT")
        self.reader_right = StreamReader(YOUTUBE_RIGHT, "RIGHT")

        ok_left = self.reader_left.open()
        ok_right = self.reader_right.open()
        if not ok_left and not ok_right:
            raise RuntimeError("No se pudo abrir ninguno de los dos directos")
        ...
        self.get_logger().info("TAB selecciona LEFT/RIGHT | WASD mueve | Q/E radio | I/K paso | M cambia CALIB/STITCH | P imprime | C captura | X o ESC sale")
    ...
    def loop(self) -> None:
        ...
        if time.time() - self.last_log > 5.0:
            ...
            self.get_logger().info(f"LEFT ok={l_ok} fail={l_fail} | RIGHT ok={r_ok} fail={r_fail} | mode={self.mode}")
            self.last_log = time.time()
...
def main(args=None):
    ...
    node = None
    try:
        node = YouTubeCalibAndStitchNode()
        rclpy.spin(node)
